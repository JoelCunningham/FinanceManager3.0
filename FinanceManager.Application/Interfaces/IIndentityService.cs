...
{
    Task<(bool Success, IEnumerable<string> Errors)> CreateUserAsync(string name, string email, string password);
    ...
    Task<(bool Success, IEnumerable<string> Errors)> ResetPasswordAsync(string email, string token, string password);
